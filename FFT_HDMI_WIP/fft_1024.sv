...
module fft_1024 #(
  ...
  // Twiddle factor index (uses only N/2 unique values)
  logic [log2N-2:0] twiddle_index;
  ...
  // bit-reversal for address sequencing
  logic [log2N-1:0] rcount;
  function automatic [log2N-1:0] reverse_bits (input [log2N-1:0] x);
    integer k;
    begin
      for (k = 0; k < log2N; k = k + 1) begin
        reverse_bits[k] = x[log2N-1-k];
      end
    end
  endfunction
  assign rcount = reverse_bits(count);
